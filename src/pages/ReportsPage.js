...
export class ReportsPage {
  readonly page: Page;
  readonly locators = {
    mainInput: '#reports-input',
    actionButton: '#reports-action',
  ...
  async gotoReports() {
    await setupTestData(this.page, { module: 'reports' });
    await this.page.goto('/reports');
    await expect(this.page).toHaveURL(/reports/);
  }

  async performAction(data = {}) {
    await this.page.fill(this.locators.mainInput, data.value || 'test-reports');
  ...
  async performComplexAction(data = {}) {
    ...
    await this.page.selectOption('#reports-select', data.option || 'option1');
